 ...
 * exclusivamente para la certificación, separado del EXIF ({@code APP1}).
 * </p>
 *
...
public class InyeccionDatosJPEGAdapter implements InyeccionDatosPort {
    ...
    public byte[] inyectar(byte[] imagenOriginal, String jsonCertificacion) {
        ...
        if (longitudSegmento > 65533) {
            throw new InyeccionDatosException("El JSON de certificación es demasiado largo para APP11.");
        }
        ...
        try {
            ...
            throw new InyeccionDatosException("Error inyectando segmento APP11 en JPEG: " + e.getMessage(), e);
        }
    ...
    private void verificarFormatoJPEG(byte[] datos) {
        if (datos == null || datos.length < 4) {
            throw new InyeccionDatosException("Los datos no son un JPEG válido.");
        }
        if ((datos[0] & 0xFF) != 0xFF || (datos[1] & 0xFF) != 0xD8) {
            throw new InyeccionDatosException("Firma JPEG inválida (SOI no encontrado). ¿Es realmente un JPEG?");
        }
    }
}
