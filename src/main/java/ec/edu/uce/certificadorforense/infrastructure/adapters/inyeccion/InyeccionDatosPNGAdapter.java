...
public class InyeccionDatosPNGAdapter implements InyeccionDatosPort {
    ...
    public byte[] inyectar(byte[] imagenOriginal, String jsonCertificacion) {
        ...
        try {
            ...
            throw new InyeccionDatosException("Error ensamblando PNG certificado: " + e.getMessage(), e);
        }
    ...
    private byte[] construirChunkTexto(String json) {
        ...
        try {
            ...
            throw new InyeccionDatosException("Error construyendo chunk tEXt", e);
        }
        return baos.toByteArray();
    }
    ...
    private int buscarPosicionIEND(byte[] png) {
        ...
        throw new InyeccionDatosException("No se encontró el chunk IEND en el PNG.");
    }

    private void verificarFormatoPNG(byte[] datos) {
        if (datos == null || datos.length < 8) {
            throw new InyeccionDatosException("Los datos no son un PNG válido.");
        }
        for (int i = 0; i < PNG_SIGNATURE.length; i++) {
            if (datos[i] != PNG_SIGNATURE[i]) {
                throw new InyeccionDatosException("Firma PNG inválida. ¿Es realmente un PNG?");
            }
        }
    }
